...
LPH_JIT_MAX(function() --// Main Cheat
	...
	local function ServerHop()
	    ...
	    local ServerMode = getgenv().ServerHopMode
	    ...
	    if Success and Response.data then
	        ...
	        if ServerMode == "Best Ping" then
	            for i = 1, math.min(#ServersList, MAX_PING_CHECKS) do
	                local Server = ServersList[i]
	        ...
	        if BestServer then
	            ...
	            warn("[Error]: No suitable servers found")
	        end
	    else
...
RunService.RenderStepped:Connect(function()
    ...
    local RespondMessageOption2 = SafeZoneUI and SafeZoneUI:FindFirstChild("2option") and SafeZoneUI["2option"]:FindFirstChild("button")    
